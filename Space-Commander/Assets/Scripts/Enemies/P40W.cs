...
//Attention ! 
//Classe réalisée par un autre membre de l'équipe





public class P40W : BaseEnemy
{
    ...
    protected override void  Update()
    {
        if(Player.instance != null)
        {
            if (!isCharging && !stunned)
                body.transform.up = -(Vector2)(Player.instance.transform.position - transform.position);
        }
    ...
    private IEnumerator A_Charge()
    {
        ...
        if(Player.instance != null)
        {
            chargeMovement = (Player.instance.transform.position - transform.position).normalized * chargeSpeed;
        }
