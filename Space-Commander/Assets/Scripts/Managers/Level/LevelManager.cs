...
public class LevelManager : MonoBehaviour
{
    ...
    public void GainLevel()
    {
        ...
        for(int i =0;i<listCardChoosen.Count;i++){
            cardPlaceList[i].transform.GetChild(0).GetComponent<Image>().sprite = listCardChoosen[i].GetComponent<Level>().setCard[0].GetComponent<UpgradeBase>().miniatureSprite;
            cardPlaceList[i].transform.GetChild(1).GetComponent<TextMeshProUGUI>().text = listCardChoosen[i].GetComponent<Level>().setCard[0].GetComponent<UpgradeBase>().description;
            cardPlaceList[i].transform.GetChild(2).GetComponent<TextMeshProUGUI>().text = listCardChoosen[i].GetComponent<Level>().setCard[0].GetComponent<UpgradeBase>().attributs;
    ...
    {
        setCard.GetComponent<Level>().setCard[0].GetComponent<IUpgrade>().UpgradePlayer();
        setCard.GetComponent<Level>().setCard.RemoveAt(0);
        if (setCard.GetComponent<Level>().setCard.Count == 0){
    ...
    public void Retry() => StartCoroutine(Load(SceneManager.GetActiveScene().name));

    public void Play() =>  SceneManager.LoadScene(2);


    public void Menu() => StartCoroutine(Load("Menu 1"));
